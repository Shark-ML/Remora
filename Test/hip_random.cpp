#define BOOST_TEST_MODULE Remora_HIP_Random
...
template<class V, class Dist>
void chi_squared_gof(matrix_expression<V, hip_tag> const& m_hip, Dist const& dist, typename V::value_type min, typename V::value_type max, std::size_t numBins){
	matrix<typename V::value_type> m = copy_to_cpu(m_hip);
...
template<class V, class Dist>
void chi_squared_gof(vector_expression<V, hip_tag> const& v_hip, Dist const& dist, typename V::value_type min, typename V::value_type max, std::size_t numBins){
	vector<typename V::value_type> v = copy_to_cpu(v_hip);
	chi_squared_gof(v,dist,min,max,numBins);
}

BOOST_AUTO_TEST_SUITE (Remora_HIP_Random)
...
BOOST_AUTO_TEST_CASE(Remora_Random_Normal) {
	...
	for(std::size_t i = 0; i != 10; ++i){
		//test kernels
		{
			vector<float, hip_tag> v(Dimensions);
			kernels::generate_normal(v, gen, -2, 9);
			chi_squared_gof(v,dist, -10, 10,20);
		}
		{
			matrix<float, row_major, hip_tag> m(77,Dimensions/77);
			kernels::generate_normal(m, gen, -2, 9);
			chi_squared_gof(m,dist, -10, 10,20);
		}
		{
			matrix<float, column_major, hip_tag> m(77,Dimensions/77);
			kernels::generate_normal(m, gen, -2, 9);
			chi_squared_gof(m,dist, -10,10,20);
		}
		//test expressions
		{
			vector<float, hip_tag> v = 3.0*normal(gen, Dimensions, -2.0/3.0, 1.0, hip_tag());
			chi_squared_gof(v,dist, -10, 10,20);
		}
		{
			vector<float, hip_tag> v(Dimensions,1);
			noalias(v) += 3.0*normal(gen, Dimensions, -1.0, 1.0, hip_tag());
			chi_squared_gof(v,dist, -10, 10,20);
		}
		{
			matrix<float, row_major, hip_tag> m = 3.0*normal(gen, 77, Dimensions/77, -2.0/3.0, 1.0, hip_tag());
			chi_squared_gof(m,dist, -10, 10,20);
		}
		{
			matrix<float, row_major, hip_tag> m(77,Dimensions/77,1);
			noalias(m) += 3.0*normal(gen, 77, Dimensions/77, -1.0, 1.0, hip_tag());
			chi_squared_gof(m,dist, -10, 10,20);
		}
	}
}
...
BOOST_AUTO_TEST_CASE(Remora_Random_uniform) {
	...
	for(std::size_t i = 0; i != 10; ++i){
		...
		//test expressions
		{
			vector<float, hip_tag> v = 3.0*uniform(gen, Dimensions, -5.0/3.0, 7.0/3.0, hip_tag());
			chi_squared_gof(v,dist,-5,7,20);
		}
		{
			vector<float, hip_tag> v(Dimensions,1);
			noalias(v) += 3.0*uniform(gen, Dimensions, -6/3.0, 6/3.0, hip_tag());
			chi_squared_gof(v,dist,-5,7,20);
		}
		{
			matrix<float, row_major, hip_tag> m = 3.0*uniform(gen, 77, Dimensions/77, -5.0/3.0, 7.0/3.0, hip_tag());
			chi_squared_gof(m,dist,-5,7,20);
		}
		{
			matrix<float, row_major, hip_tag> m(77,Dimensions/77,1);
			noalias(m) += 3.0*uniform(gen, 77, Dimensions/77, -6/3.0, 6/3.0, hip_tag());
			chi_squared_gof(m,dist,-5,7,20);
		}
	}
}
...
BOOST_AUTO_TEST_CASE(Remora_Random_discrete) {
	std::cout<<"testing discrete distribution"<<std::endl;
