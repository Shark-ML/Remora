#define BOOST_TEST_MODULE Remora_OPENCL_Conv2d
...
void test(
	...
	//copy to Device
	vector<float,opencl_tag> filter_opencl = copy_to_opencl(filter);
	matrix<float,row_major, opencl_tag> image_opencl = copy_to_opencl(image);
	...
	matrix<float,row_major,opencl_tag> out_opencl(num_images, output_size1 * output_size2 * num_filters, 0.0);
	
	
	//compute baseline and opencl result
	kernels::conv2d(
	...
	kernels::conv2d(
		image_opencl,filter_opencl,out_opencl,num_channels, num_filters, 
		image_size1, image_size2, filter_size1, filter_size2,
		padding_height, padding_width
	);
	
	//copy result back and test
	matrix<float,row_major> out_cpu = copy_to_cpu(out_opencl);
