#define BOOST_TEST_MODULE Remora_OPENCL_MatrixAssign
...
BOOST_AUTO_TEST_CASE( Remora_Matrix_Assign_Dense ){
	...
	matrix<float, row_major, opencl_tag> source = copy_to_opencl(source_cpu);
	matrix<float, column_major, opencl_tag> source_cm = copy_to_opencl(source_cpu);
	matrix<float, row_major, opencl_tag> result_add = copy_to_opencl(result_add_cpu);
	matrix<float, row_major, opencl_tag> result_add_scalar = copy_to_opencl(result_add_scalar_cpu);
	{
		std::cout<<"testing direct assignment row-row"<<std::endl;
		matrix<float, row_major, opencl_tag> target = copy_to_opencl(target_cpu);
	...
	}
	{
		...
		matrix<float, row_major, opencl_tag> target = copy_to_opencl(target_cpu);
		kernels::assign(target,source, device_traits<opencl_tag>::add<float>());
	...
	}
	{
		...
		matrix<float, row_major, opencl_tag> target = copy_to_opencl(target_cpu);
		kernels::assign(target,source_cm, device_traits<opencl_tag>::add<float>());
		checkMatrixEqual(target,result_add);
	}
	{
		std::cout<<"testing functor scalar assignment"<<std::endl;
		matrix<float, row_major, opencl_tag> target = copy_to_opencl(target_cpu);
		kernels::assign<device_traits<opencl_tag>::add<float> >(target,scalar);
		target.queue().finish();
