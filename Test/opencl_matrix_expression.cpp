#define BOOST_TEST_MODULE Remora_OPENCL_Matrix_Expression
...
void checkDenseExpressionEquality(
	matrix_expression<Operation, opencl_tag> const& op_opencl, Result const& result
){
	BOOST_REQUIRE_EQUAL(op_opencl().size1(), result.size1());
	BOOST_REQUIRE_EQUAL(op_opencl().size2(), result.size2());
	
	//check that matrix assignment using op() works(implicit test)
	matrix<float> op = copy_to_cpu(op_opencl());
	...
	{
		matrix<float, row_major, opencl_tag> res1_opencl(result.size1(),result.size2(),0.1f);
		matrix<float, row_major, opencl_tag> res2_opencl(result.size1(),result.size2(),0.1f);
		op_opencl().assign_to(res1_opencl);
		op_opencl().plus_assign_to(res2_opencl);
		matrix<float> res1 = copy_to_cpu(res1_opencl);
		matrix<float> res2 = copy_to_cpu(res2_opencl);
		for(std::size_t i = 0; i != op.size1(); ++i){
			for(std::size_t j = 0; j != op.size2(); ++j){
	...
	{
		matrix<float, column_major, opencl_tag> res1_opencl(result.size1(),result.size2(),0.1f);
		matrix<float, column_major, opencl_tag> res2_opencl(result.size1(),result.size2(),0.1f);
		op_opencl().assign_to(res1_opencl);
		op_opencl().plus_assign_to(res2_opencl);
		matrix<float, column_major> res1 = copy_to_cpu(res1_opencl);
		matrix<float, column_major> res2 = copy_to_cpu(res2_opencl);
		for(std::size_t i = 0; i != op.size1(); ++i){
			for(std::size_t j = 0; j != op.size2(); ++j){
				BOOST_CHECK_CLOSE(result(i,j), res1(i,j),1.e-3);
				BOOST_CHECK_CLOSE(result(i,j) + 0.1f, res2(i,j),1.e-2);
...
template<class Operation, class Result>
void checkDenseExpressionEquality(
	vector_expression<Operation, opencl_tag> const& op_opencl, Result const& result
){
	BOOST_REQUIRE_EQUAL(op_opencl().size(), result.size());
	
	vector<float> op = copy_to_cpu(op_opencl());
...
BOOST_AUTO_TEST_CASE( Remora_matrix_Outer_Prod ){
	...
	vector<float, opencl_tag> x = copy_to_opencl(x_cpu);
	vector<float, opencl_tag> y = copy_to_opencl(y_cpu);
...
BOOST_AUTO_TEST_CASE( Remora_matrix_Unary_Minus )
{
	...
	matrix<float, row_major, opencl_tag> x = copy_to_opencl(x_cpu);
...
BOOST_AUTO_TEST_CASE( Remora_matrix_Binary_Plus)
{
	...
	matrix<float, row_major, opencl_tag> x = copy_to_opencl(x_cpu);
	matrix<float, row_major, opencl_tag> y = copy_to_opencl(y_cpu);
...
BOOST_AUTO_TEST_CASE( Remora_trace){
	...
	matrix<float, row_major, opencl_tag> x_row = copy_to_opencl(x_cpu);
	matrix<float, column_major, opencl_tag> x_col = copy_to_opencl(x_cpu);
...
BOOST_AUTO_TEST_CASE( Remora_frobenius_prod){
	...
	matrix<float, row_major, opencl_tag> x_row = copy_to_opencl(x_cpu);
	matrix<float, column_major, opencl_tag> x_col = copy_to_opencl(x_cpu);
	matrix<float, row_major, opencl_tag> y_row = copy_to_opencl(y_cpu);
	matrix<float, column_major, opencl_tag> y_col = copy_to_opencl(y_cpu);
