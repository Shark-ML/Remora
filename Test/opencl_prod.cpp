#define BOOST_TEST_MODULE Remora_opencl_prod
...
void checkMatrixVectorMultiply(M const& arg1_opencl, V const& arg2_opencl, Result const& result_opencl, float factor, float init = 0){
	BOOST_REQUIRE_EQUAL(arg1_opencl.size1(), result_opencl.size());
	BOOST_REQUIRE_EQUAL(arg2_opencl.size(), arg1_opencl.size2());
	
	matrix<float> arg1 = copy_to_cpu(arg1_opencl);
	vector<float> arg2 = copy_to_cpu(arg2_opencl);
	vector<float> result = copy_to_cpu(result_opencl); 
...
BOOST_AUTO_TEST_SUITE (Remora_opencl_prod)

BOOST_AUTO_TEST_CASE( Remora_opencl_prod_vector_dense ){
	std::size_t rows = 50;
	...
	matrix<float,row_major, opencl_tag> arg1rm = copy_to_opencl(arg1rm_cpu);
	matrix<float,column_major, opencl_tag> arg1cm = copy_to_opencl(arg1cm_cpu);
	matrix<float,row_major, opencl_tag> arg1rmt = copy_to_opencl(arg1rmt_cpu);
	matrix<float,column_major, opencl_tag> arg1cmt = copy_to_opencl(arg1cmt_cpu);
	vector<float, opencl_tag> arg2 = copy_to_opencl(arg2_cpu);
	...
	//test first expressions of the form A += alpha*B*C 
	{
		...
		vector<float, opencl_tag> result(rows,1.5);
...
void checkMatrixMatrixMultiply(Arg1 const& arg1_opencl, Arg2 const& arg2_opencl, Result const& result_opencl, float factor, float init = 0){
	BOOST_REQUIRE_EQUAL(arg1_opencl.size1(), result_opencl.size1());
	BOOST_REQUIRE_EQUAL(arg2_opencl.size2(), result_opencl.size2());
	
	matrix<float> arg1 = copy_to_cpu(arg1_opencl);
	matrix<float> arg2 = copy_to_cpu(arg2_opencl);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(Remora_prod_opencl_matrix_dense_dense, Orientation,result_orientations) {
	std::size_t rows = 50;
	...
	matrix<float,row_major, opencl_tag> arg1rm = copy_to_opencl(arg1rm_cpu);
	matrix<float,column_major, opencl_tag> arg1cm = copy_to_opencl(arg1cm_cpu);
	matrix<float,row_major, opencl_tag> arg2rm = copy_to_opencl(arg2rm_cpu);
	matrix<float,column_major, opencl_tag> arg2cm = copy_to_opencl(arg2cm_cpu);
	
	std::cout<<"\nchecking dense-dense matrix-matrix plusassign multiply"<<std::endl;
	//test first expressions of the form A+=B*C
	{
		std::cout<<"rrr"<<std::endl;
		matrix<float,row_major, opencl_tag> resultrm(rows,columns,1.5);
		noalias(resultrm) += -2.0 * prod(arg1rm,arg2rm);
		checkMatrixMatrixMultiply(arg1rm,arg2rm,resultrm,-2.0,1.5);
	}
	{
		std::cout<<"rrc"<<std::endl;
		matrix<float,column_major, opencl_tag> resultcm(rows,columns,1.5);
