#define BOOST_TEST_MODULE Remora_OPENCL_Syrk
...
#include <remora/opencl/copy.hpp>
...
BOOST_AUTO_TEST_CASE_TEMPLATE(syrk_test, Orientation,result_orientations) {
	...
	matrix<float,row_major, opencl_tag> argrm = copy_to_opencl(arg_cpu);
	matrix<float,column_major, opencl_tag> argcm = copy_to_opencl(arg_cpu);

	std::cout << "\nchecking syrk V+=AA^T" << std::endl;
	{
		std::cout<<"row major A, lower V"<<std::endl;
		matrix<float, Orientation, opencl_tag> result(dims,dims,3.0);
