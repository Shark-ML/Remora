#define BOOST_TEST_MODULE Remora_opencl_triangular_prod
...
void checkMatrixVectorMultiply(M const& arg1, V const& arg2_opencl, Result const& result_opencl,float init, float alpha){
	BOOST_REQUIRE_EQUAL(arg1.size1(), result_opencl.size());
	BOOST_REQUIRE_EQUAL(arg2_opencl.size(), arg1.size2());
...
void checkMatrixMatrixMultiply(M1 const& arg1, M2 const& arg2_opencl, Result const& result_opencl,float init, float alpha) {
	BOOST_REQUIRE_EQUAL(arg1.size1(), arg1.size2());
	BOOST_REQUIRE_EQUAL(arg1.size2(), arg2_opencl.size1());
	BOOST_REQUIRE_EQUAL(arg1.size2(), result_opencl.size1());
...
BOOST_AUTO_TEST_SUITE(Remora_opencl_triangular_prod)

BOOST_AUTO_TEST_CASE(Remora_opencl_triangular_prod_matrix_vector) {
	...
	matrix<float, row_major, opencl_tag> arg1lowerrm = copy_to_opencl(arg1lowerrm_cpu);
	matrix<float, column_major, opencl_tag> arg1lowercm = copy_to_opencl(arg1lowercm_cpu);
	matrix<float, row_major, opencl_tag> arg1upperrm = copy_to_opencl(arg1upperrm_cpu);
	matrix<float, row_major, opencl_tag> arg1uppercm = copy_to_opencl(arg1uppercm_cpu);
	...
	std::cout<<"\nchecking matrix-vector prod v=Ax non-unit"<<std::endl;
	{
		...
		vector<float, opencl_tag> result(dims,3.0);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(Remora_triangular_prod_matrix_matrix, Orientation,result_orientations) {
	...
	matrix<float, row_major, opencl_tag> arg1upperrm = copy_to_opencl(arg1upperrm_cpu);
	matrix<float, row_major, opencl_tag> arg1uppercm = copy_to_opencl(arg1uppercm_cpu);
	matrix<float, column_major, opencl_tag> arg2 = copy_to_opencl(arg2_cpu);

	std::cout << "\nchecking matrix-matrix prod V=AX non-unit" << std::endl;
	{
		std::cout<<"row major lower AX"<<std::endl;
		matrix<float, Orientation, opencl_tag> result(dims, N, 3.0);
		noalias(result) = triangular_prod<lower>(arg1lowerrm,arg2);
