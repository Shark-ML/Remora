#define BOOST_TEST_MODULE Remora_OPENCL_vector_expression
...
void checkDenseExpressionEquality(
	Operation op_opencl, Result const& result
){
	...
	vector<float, opencl_tag> res1_opencl(result.size(),0.01f);
	vector<float, opencl_tag> res2_opencl(result.size(),0.01f);
	op_opencl.assign_to(res1_opencl);
	op_opencl.plus_assign_to(res2_opencl);
	vector<float> res1 = copy_to_cpu(res1_opencl);
	vector<float> res2 = copy_to_cpu(res2_opencl);
	for(std::size_t i = 0; i != op.size(); ++i){
