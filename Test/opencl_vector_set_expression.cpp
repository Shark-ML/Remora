#define BOOST_TEST_MODULE Remora_OPENCL_Vector_Set_Expression
...
template<class Operation, class Result>
void checkDenseExpressionEquality(
	vector_set_expression<Operation, opencl_tag> const& op_opencl, Result const& result
){
	matrix<float, row_major, opencl_tag> mat_opencl = op_opencl;
	matrix<float> op = copy_to_cpu(mat_opencl);
...
BOOST_AUTO_TEST_CASE_TEMPLATE( Remora_sum_set, Orientation, result_orientations )
{
	...
	matrix<float, Orientation, opencl_tag> x = copy_to_device(x_cpu, opencl_tag());
