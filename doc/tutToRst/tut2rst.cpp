...
void help()
{
	cerr << "usage: ./tut2rst <filename> <path>" << endl;
}
...
int main(int argc, char** argv)
{
	try
	{
		...
		// process
		size_t start = 0;
		std::string tag = "..includecode<";
		while (true)
		{
			size_t pos = input.find(tag, start);
			if (pos == string::npos)
			{
			...
			start = closing + 1;
			if (closing == string::npos) throw string("includecode tag not closed (missing '>')");
			if (comma > closing)
			{
			...
			else
			{
				...
				while (true)
				{
					...
					while (true)
					{
						...
						if (end == string::npos) throw string("includecode tag not closed (missing '>')");
						end++;
						if (snippet[end] == '\n') end++;
